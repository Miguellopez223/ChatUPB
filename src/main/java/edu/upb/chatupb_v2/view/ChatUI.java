...
public class ChatUI extends JFrame implements IChatView {
    ...
    public ChatUI() {
        ...
        this.contactController = new ContactController(this); // This will be managed by ChatController
        configurarVentana();
    }
    ...
    public void abrirChatConContacto(ContactInfo contacto, List<ChatMessageInfo> historial) {
        ...
        JTextArea area = chatAreas.computeIfAbsent(contacto.getIp(), k -> new JTextArea());
    ...
    public void appendChatToContact(String ip, String texto) {
        JTextArea area = chatAreas.computeIfAbsent(ip, k -> new JTextArea());
        area.setEditable(false);
        area.append(texto);
